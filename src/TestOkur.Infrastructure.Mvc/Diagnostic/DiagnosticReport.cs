...
namespace TestOkur.Infrastructure.Mvc.Diagnostic
{
    ...
    public class DiagnosticReport
    {
        ...
        {
            return !IsReportEnabled(type, DiagnosticReportType.System)
                ? null
                : new SystemDetails
            {
        ...
        {
            return !IsReportEnabled(type, DiagnosticReportType.EnvironmentVariables)
                ? null
        ...
        {
            return !IsReportEnabled(type, DiagnosticReportType.Drives)
                ? null
                : DriveInfo.GetDrives()
                .Select(d =>
                {
        ...
        {
            return !IsReportEnabled(type, DiagnosticReportType.Assemblies)
                ? null
        ...
        {
            if (!IsReportEnabled(type, DiagnosticReportType.Networks))
            {
                return null;
            }
        ...
        private static string GenerateImpl(DiagnosticReport report)
        {
            ...
            builder.Insert(
                0,
                $"/{NewLine}{Pipe}Diagnostic Report generated at: " +
        ...
        private static string GetProcessorName()
        {
            ...
            try
            {
                if (ApplicationHelper.IsWindows)
                {
                    result = GetProcessorNameWindows();
                }

                if (ApplicationHelper.IsLinux)
                {
                    result = GetProcessorNameLinux();
                }

                if (ApplicationHelper.IsOSX)
                {
                    result = GetProcessorNameOSX();
                }
        ...
        private static long GetInstalledMemoryInGigaBytes()
        {
            try
            {
                if (ApplicationHelper.IsWindows)
                {
                    return GetInstalledMemoryInGigaBytesWindows();
                }

                return ApplicationHelper.IsLinux
                    ? GetInstalledMemoryInGigaBytesLinux()
                    : ApplicationHelper.IsOSX ? GetInstalledMemoryInGigaBytesOSX() : 0;
            }
            catch (Exception)
            {
                return 0;
        ...
        private static TimeSpan GetUptime()
        {
            try
            {
                return ApplicationHelper.IsWindows ? GetUptimeWindows() : ApplicationHelper.IsLinux ? GetUptimeLinux() : TimeSpan.MinValue;
            }
            catch (Exception)
            {
        ...
        private static string GetOSPlatform()
        {
            ...
            return ApplicationHelper.OSPlatform == OSPlatform.Linux
                ? "Linux"
                : ApplicationHelper.OSPlatform == OSPlatform.OSX ? "OSX" : ApplicationHelper.OSPlatform.ToString();
        }

        private static bool IsWindowsServer() => IsOS(OsAnyserver);
